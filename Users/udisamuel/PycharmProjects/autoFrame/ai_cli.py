...
try:
    ...
    logger.warning("OpenAI package not found. Using fallback methods for AI capabilities.")
...
def main():
    ...
    if not Config.AI_FEATURES_ENABLED:
        logger.warning("AI features are disabled. Enable them by setting AI_FEATURES_ENABLED=true in your .env file.")
        logger.info("Continuing with fallback methods where available.")
    
    # Check if OpenAI API key is configured when OpenAI is available
    if OPENAI_AVAILABLE and not Config.OPENAI_API_KEY:
        logger.warning("OpenAI API key is not configured. Set OPENAI_API_KEY in your .env file for optimal results.")
    ...
    if args.command == "generate-data":
        ...
        if args.constraints:
            try:
                ...
                logger.error(f"Error: Invalid JSON in constraints: {args.constraints}")
                return
        ...
        if args.type == "user":
            ...
            if not args.endpoint:
                logger.error("Error: --endpoint is required for API data generation")
                return
            ...
            if not args.form_name or not args.fields:
                logger.error("Error: --form-name and --fields are required for form data generation")
                return
            ...
            if not args.fields:
                logger.error("Error: --fields is required for dataset generation")
                return
        ...
        if args.type == "api":
            if not args.endpoint:
                logger.error("Error: --endpoint is required for API test generation")
                return
            ...
            if not args.page_name:
                logger.error("Error: --page-name is required for UI test generation")
                return
                
            if not args.steps:
                logger.error("Error: --steps is required for UI test generation")
                return
            ...
            if not args.db_type or not args.query:
                logger.error("Error: --db-type and --query are required for DB test generation")
                return
        ...
        logger.info(f"Test saved to {file_path}")
        ...
        if args.response:
            try:
                ...
                logger.warning(f"Warning: Error reading response data file: {e}")
